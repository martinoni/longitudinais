...
fit_1 <- lme(diam ~ I(as.numeric(grupo == 'AIG')) + 
               ...
               I(as.numeric(I(grupo == 'AIG'))*semana^2) +
               I(as.numeric(I(grupo == 'PIG'))*semana^2) +
               I(as.numeric(I(grupo == 'AIG'))*semana^3) +
               I(as.numeric(I(grupo == 'PIG'))*semana^3)- 1 , random = ~semana + I(semana^2) | id, data=DDVE.l,
...
ggplot(data=DDVE.l, aes(x=semana, y=diam, group = grupo,color=grupo)) +
  stat_function(fun=function(x) 179.49846 -13.62640*x + 0.36996*x^2 -0.00342*x^3, colour = "blue") +
  stat_function(fun=function(x) -147.53562 + 15.33610 *x -0.47047*x^2 +0.00461*x^3, colour = "red") +
...
ggplot(data=DDVE.l1, aes(x=semana, y=diam )) +
  stat_function(fun=function(x) 179.49846 -13.62640*x + 0.36996*x^2 -0.00342*x^3, colour = "blue") +
  stat_function(fun=function(x) 176.8206 -13.3157*x + 0.3641245*x^2 -0.00342*x^3, colour = "black") +
...
DDVE.l47 <- subset(DDVE.l, id==47)
ggplot(data=DDVE.l47, aes(x=semana, y=diam )) +
  stat_function(fun=function(x) -147.53562 + 15.33610*x -0.47047*x^2 +0.00461*x^3, colour = "red") +
  stat_function(fun=function(x) -134.5821 + 14.37078*x -0.4549729*x^2 +0.00461*x^3, colour = "black") +
...
DDVE.l58 <- subset(DDVE.l, id==58)
ggplot(data=DDVE.l58, aes(x=semana, y=diam )) +
  stat_function(fun=function(x) -147.53562 + 15.33610*x -0.47047*x^2 +0.00461*x^3, colour = "red") +
  stat_function(fun=function(x) -153.3321 + 15.89311*x -0.4803659*x^2 +0.00461*x^3, colour = "black") +
  geom_point(color="red")+
  scale_x_continuous(limits = c(26, 40)) + 
  scale_y_continuous(limits = c(6, 20)) +
  scale_color_manual(values = c("blue","red")) 
...
library(nlme)
fit_2 <- lme(diam ~ I(as.numeric(grupo == 'AIG')) + 
               I(as.numeric(grupo == 'PIG')) +
               I(as.numeric(I(grupo == 'AIG'))*semana) +
               I(as.numeric(I(grupo == 'PIG'))*semana) +
               I(as.numeric(I(grupo == 'AIG'))*semana^2) +
